                ...
                <div class="middle-content container-xxl p-0">
                    ...
                    <div class="page-meta">
                        <nav class="breadcrumb-style-one" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="app-additions.php">Adisyonlar</a></li>
                                <li class="breadcrumb-item active" aria-current="page">Adisyon Kayıtları</li>
                            </ol>
                        </nav>
                    </div>
                    <!-- /BREADCRUMB -->
                            ...
                            <div class="widget-content widget-content-area br-8">
                                <table id="zero-config" class="table table-striped dt-table-hover" style="width:100%">
                                    <thead>
                                        ...
                                        <tr>
                                            <th class="no-content">No</th>
                                            <th class="no-content">Sipariş Tarihi</th>
                                            <th class="no-content">Kullanıcı</th>
                                            <th class="no-content">Masa/Tip</th>
                                            <th class="no-content">İşletme</th>
                                            <th class="no-content">Ödeme Tipi</th>
                                            <th class="no-content">Tutar</th>
                                            <th class="no-content">Durumu</th>
                                    ...
                                    <tbody>
                                        <?php
                                        if (is_array($fetchDataOrdersForAdditions)) {
                                            $sn = 1;
                                            foreach ($fetchDataOrdersForAdditions as $data) {

                                                $order_items_table_order_id = $data['id'];
                                                $fetchDataOrderItemsTableDetail = fetch_data_order_items_for_additions($db, $tableNameOrderItems, $columnsOrderItems, $order_items_table_order_id);
                                                $fetchDataOrderItemsTotalPriceTableDetail = fetch_data_order_items_total_price_for_additions($db, $tableNameOrderItems, $columnsOrderItems, $order_items_table_order_id);
                                                $users_table_users_id = $fetchDataOrderItemsTableDetail['users_id'];
                                                $business_table_business_id = $fetchDataOrderItemsTableDetail['business_id'];
                                                $payment_type_table_payment_type_id = $fetchDataOrderItemsTableDetail['payment_type'];
                                                $fetchDataUsersTableDetail = fetch_data_users_for_additions($db, $tableNameUsers, $columnUsers, $users_table_users_id);
                                                $fetchDataBusinessTableDetail = fetch_data_business_for_additions($db, $tableNameBusiness, $columnsBusinessCategory, $business_table_business_id);
                                                $fetchDataPaymentTypeTableDetail = fetch_data_payment_type_for_additions($db, $tableNamePaymentTypes, $columnsPaymentTypes, $payment_type_table_payment_type_id);

                                                $order_id = $data['id'];
                                                $order_type = $data['order_type'];
                                                $desk_number = $data['desk_number'];
                                                
                                                $order_type = cleanTurkishCharacters($order_type);
                                                $desk_number = cleanTurkishCharacters($desk_number);

                                                // SEO uyumlu URL'yi oluştur
                                                $url = "/$order_type" . "/$desk_number" . "/$order_id";
                                        ?>
                                                <tr>
                                                    <td><?php echo $sn ?></td>
                                                    <td><?php echo $data['created_at'] ?></td>
                                                    <td><?php echo $fetchDataUsersTableDetail['users_name'] ?></td>
                                                    <td><?php echo $data['order_type'] ?></td>
                                                    <td><?php echo $fetchDataBusinessTableDetail['business_name'] ?></td>
                                                    <td><?php echo $fetchDataPaymentTypeTableDetail['payment_types_name'] ?></td>
                                                    <td>₺<?php echo number_format($fetchDataOrderItemsTotalPriceTableDetail, 2); ?></td>
                                                    <td>
    <?php
    if ($data['order_activity'] == 0) {
        echo 'Kapalı';
    } elseif ($data['order_activity'] == 1) {
        echo 'Açık';
    } else {
        // Opsiyonel: Eğer 0 veya 1 dışında bir değer gelirse, burada bir hata mesajı veya varsayılan bir değer yazabilirsiniz.
        echo 'Bilinmeyen Durum';
    }
    ?>
</td>                                                    <td>
                                                        <a href="app-additions-detail.php?<?php echo $url ?>">
                                                            <svg width="24px" height="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                                <g id="SVGRepo_iconCarrier">
                                        ...
                                        <?php
                                            ...
                                            echo $fetchDataOrdersForAdditions;
                                        } ?>
                                    </tbody>
                                    <tfoot>
                                        <tr>
                                            <th class="no-content">No</th>
                                            <th class="no-content">Açılış/Kapanış</th>
                                            <th class="no-content">Kullanıcı</th>
                                            <th class="no-content">Masa/Tip</th>
                                            <th class="no-content">İşletme</th>
                                            <th class="no-content">Ödeme Tipi</th>
