...
<head>
    ...
    <title>Stok | Asperula Kafe - Sipariş Sistemi Paneli </title>
                ...
                <div class="middle-content container-xxl p-0">
                    ...
                    <div class="page-meta">
                        <nav class="breadcrumb-style-one" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="app-stock.php">Stok</a></li>
                                <li class="breadcrumb-item active" aria-current="page">Stok Ürünler</li>
                            </ol>
                        </nav>
                    </div>
                    <!-- /BREADCRUMB -->
                            ...
                            <div class="widget-content widget-content-area br-8">
                                <table id="zero-config" class="table table-striped dt-table-hover" style="width:100%">
                                    <div class="col-md-12 col-sm-12 col-12 m-2">
                                        <a class="btn btn-primary" href="app-stock-add.php">
                                    ...
                                    <tbody>
                                        <?php
                                        if (is_array($fetchDataStock)) {
                                            $sn = 1;
                                            foreach ($fetchDataStock as $data) {

                                                $for_other_material_table_id = $data['material_id'];
                                                $fetchDataMaterialTableForStock = fetch_data_materialTable_for_stock($db, $tableNameMaterial, $columnsMaterial, $for_other_material_table_id);

                                                $for_other_storage_table_id = $data['storage_id'];
                                                $fetchDataStorageTableForStock = fetch_data_storageTable_for_stock($db, $tableNameStorage, $columnsStorage, $for_other_storage_table_id);

                                                $storage_name_url = $fetchDataStorageTableForStock['storage_name'];
                                                $material_name_url = $fetchDataMaterialTableForStock['material_name'];
                                                $stock_id_url = $data['stock_id'];

                                                $seo_name_storage = cleanTurkishCharacters($storage_name_url);
                                                $seo_name_material = cleanTurkishCharacters($material_name_url);

                                                // SEO uyumlu URL'yi oluştur
                                                $url = "/$seo_name_storage" . "/$seo_name_material" . "/$stock_id_url";
                                        ?>
                                                <tr>
                                                    <td>
                                                        <div class="d-flex">
                                                            <p class="align-self-center mb-0 admin-name"> <?php echo $fetchDataMaterialTableForStock['material_name'] ?> </p>
                                                        </div>
                                                    </td>
                                                    <td><?php echo $fetchDataMaterialTableForStock['material_unit'] ?></td>
                                                    <td><?php echo $fetchDataStorageTableForStock['storage_name'] ?></td>
                                                    <td><?php echo $data['stock_processing'] ?></td>
                                                    <td><?php echo ($fetchDataMaterialTableForStock['material_unit_amount'] . " " . $fetchDataMaterialTableForStock['material_unit']) ?></td>
                                                    <td><?php echo ($data['stock_amount'] . " " . $fetchDataMaterialTableForStock['material_unit']) ?></td>
                                                    <td>
                                                        <?php
                                                        $total = $fetchDataMaterialTableForStock['material_unit_amount'] - $data['stock_amount'];
                                                        $total = $total . " " . $fetchDataMaterialTableForStock['material_unit'];
                                                        echo $total;
                                                        ?>
                                                    </td>
                                                    <td> <?php echo $data['stock_price'] ?> ₺</td>
                                                    <td><?php
                                                        if (isset($data['stock_publicy'])) {
                                                            if ($data['stock_publicy'] == 1) {
                                                                echo '<span class="badge badge-light-info inv-status">Aktif</span>';
                                                            } else {
                                                                echo '<span class="badge badge-light-warning inv-status">Pasif</span>';
                                                            }
                                                        }
                                                        ?></td>
                                                    <td> <?php echo $data['stock_currentDateTime'] ?> </td>
                                                    <td>
                                                        <a href="app-stock-edit.php?<?php echo $url ?>">
                                                            <svg width="24px" height="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                                                <g id="SVGRepo_iconCarrier">
